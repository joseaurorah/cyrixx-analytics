...
<div class="altum-animate altum-animate-fill-none altum-animate-fade-in">
    <div class="d-flex justify-content-between mb-2">
        <small class="text-muted font-weight-bold text-uppercase"><?= l('dashboard.browser_names.browser') ?></small>
    ...
    <?php else: ?>
        ...
        <?php if($data->total_rows > count($data->rows)): ?>
            <a href="<?= url('dashboard/browser-names') ?>"><?= sprintf(l('global.view_x_more'), nr($data->total_rows - count($data->rows))) ?></a>
        <?php endif ?>
