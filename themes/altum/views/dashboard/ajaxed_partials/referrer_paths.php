...
<div class="altum-animate altum-animate-fill-none altum-animate-fade-in">
    ...
    <?php else: ?>
            ...
            <div class="mb-3">
                <div class="d-flex justify-content-between mb-1">
                    <div class="text-truncate">
                        ...
                        <?php endif ?>
                        <span title="<?= $row->referrer_path ?? l('dashboard.referrer_paths.null') ?>" class="align-middle"><?= $row->referrer_path ?? l('dashboard.referrer_paths.null') ?></span>
                        <a href="<?= 'https://' . $row->referrer_host . $row->referrer_path ?>" target="_blank" rel="nofollow noopener" class="text-muted ml-1"><i class="fa fa-fw fa-xs fa-external-link-alt"></i></a>
                    </div>
        ...
        <?php if($data->total_rows > count($data->rows)): ?>
            <a href="<?= url('dashboard/referrers') ?>"><?= sprintf(l('global.view_x_more'), nr($data->total_rows - count($data->rows))) ?></a>
        <?php endif ?>
