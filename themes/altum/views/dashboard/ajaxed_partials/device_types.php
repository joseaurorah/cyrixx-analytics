...
<div class="altum-animate altum-animate-fill-none altum-animate-fade-in">
    <div class="d-flex justify-content-between mb-2">
        <small class="text-muted font-weight-bold text-uppercase"><?= l('dashboard.device_types.device_type') ?></small>
    ...
    <?php else: ?>
        ...
        <?php foreach($data->rows as $row): ?>
            ...
            <div class="mb-3">
                <div class="d-flex justify-content-between mb-1">
                    <div>
                        <span><i class="fa fa-fw fa-<?= $row->device_type ?> fa-sm mr-1"></i> <?= l('global.device.' . $row->device_type) ?></span>
                    </div>
        ...
        <?php if($data->total_rows > count($data->rows)): ?>
            <a href="<?= url('dashboard/device-types') ?>"><?= sprintf(l('global.view_x_more'), nr($data->total_rows - count($data->rows))) ?></a>
        <?php endif ?>
