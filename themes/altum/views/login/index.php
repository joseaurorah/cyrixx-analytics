...
<form action="" method="post" class="mt-4" role="form">
    <?php if(isset($_SESSION['twofa_required']) && $data->user && $data->user->twofa_secret && $data->user->status == 1): ?>
        ...
        <div class="form-group mt-3">
            <button type="submit" name="submit" class="btn btn-primary btn-lg btn-block my-1"><?= l('login.form.verify') ?></button>
        </div>
    <?php else: ?>

        <div class="form-group">
            <label for="email"><?= l('login.form.email') ?></label>
            <input id="email" type="text" name="email" class="form-control form-control-lg <?= \Altum\Alerts::has_field_errors('email') ? 'is-invalid' : null ?>" value="<?= $data->values['email'] ?>" required="required" autofocus="autofocus" />
            <?= \Altum\Alerts::output_field_error('email') ?>
        </div>

        <div class="form-group">
            <label for="password"><?= l('login.form.password') ?></label>
            <input id="password" type="password" name="password" class="form-control form-control-lg <?= \Altum\Alerts::has_field_errors('password') ? 'is-invalid' : null ?>" value="<?= $data->user ? $data->values['password'] : null ?>" required="required" />
        ...
        <div class="form-group mt-4">
            <button type="submit" name="submit" class="btn btn-primary btn-lg btn-block my-1"><?= l('login.form.login') ?></button>
        </div>
    <?php endif ?>
