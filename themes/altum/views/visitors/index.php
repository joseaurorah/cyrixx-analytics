...
<header class="header">
    <div class="container">
        ...
        <div class="d-flex flex-column flex-xl-row justify-content-between mb-3">
            <div>
                <h1 class="h3 text-break"><?= sprintf(l('visitors.header.header'), $this->website->host . $this->website->path) ?></h1>
            </div>
            ...
            <div class="col-12 col-lg-4 mb-3 mb-md-0">
                <div class="card border-0">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="d-flex flex-column">
                                <small class="text-muted text-uppercase font-weight-bold"><?= l('visitors.header.average_time_per_session') ?></small>
        ...
        <div class="card border-0">
            <div class="card-body">
                <div class="d-flex flex-column align-items-center justify-content-center py-3">
                    <img src="<?= ASSETS_FULL_URL . 'images/no_rows.svg' ?>" class="col-10 col-md-7 col-lg-4 mb-3" alt="<?= l('visitors.basic.no_data') ?>" />
                    <h2 class="h4 text-muted"><?= l('visitors.basic.no_data') ?></h2>
                    <p class="text-muted"><?= sprintf(l('visitors.basic.no_data_help'), '<a href="' . url('websites') . '">', '</a>') ?></a></p>
                </div>
            </div>
        </div>
    <?php else: ?>
            ...
            <table class="table table-custom">
                ...
                <?php foreach($data->visitors as $row): ?>
                    ...
                    <tr data-visitor-id="<?= $row->visitor_id ?>">
                        ...
                        <td class="text-nowrap">
                            <div>
                                <span class="mr-1" data-toggle="tooltip" title="<?= l('global.device.' . $row->device_type) ?>"><i class="fa fa-fw fa-<?= $row->device_type ?>"></i></span>
                        ...
                        <td class="text-nowrap">
                            <a href="<?= url('visitor/' . $row->visitor_id) ?>">
                                <i class="fa fa-fw fa-hourglass-half fa-sm text-muted"></i> <?= sprintf(l('visitors.visitor.total_sessions'), '<strong>' . nr($row->total_sessions) . '</strong>') ?>
                            </a>
                        </td>
                            ...
                            <div class="d-flex justify-content-end">
                                <span data-toggle="tooltip" title="<?= l('global.delete') ?>">
                                    <button
                                            ...
                                            data-toggle="modal"
                                            data-target="#visitor_delete_modal"
                                            data-visitor-id="<?= $row->visitor_id ?>"
                                    >
                                        <i class="fa fa-fw fa-sm fa-trash-alt"></i>
                                    </button>
                                </span>
                            </div>
                        </td>
